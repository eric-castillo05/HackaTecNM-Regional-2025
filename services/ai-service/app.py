...
if __name__ == '__main__':
    ...
    # Detectar si estamos corriendo como subprocess
    import sys
    debug_mode = len(sys.argv) == 1  # Solo debug si se ejecuta directamente
    
    try:
        app.run(debug=debug_mode, host='0.0.0.0', port=8051)
    except OSError as e:
        if "Address already in use" in str(e):
            print("❌ Error: Puerto 8051 ya está en uso")
            print("💡 Solución: Ejecuta 'lsof -ti :8051 | xargs kill -9' para liberar el puerto")
            sys.exit(1)
        else:
